...
| 📦 | **离线快照** | 生成一个自包含的 `data:text/html` 链接——只读、带主题切换，无需任何服务器即可打开。快照内联全部 CSS 与图标，零外部资源，断网也能打开。 |
| 📋 | **一键复制** | 使用 Clipboard API，非安全上下文下自动回退到 `execCommand`。 |
| 📱 | **响应式布局** | 手机、平板、桌面端均可良好使用。 |
| 📴 | **完全离线** | 不依赖任何 CDN / 外部请求。直接双击 `index.html`（`file://`）即可使用——编辑、保存、分享全部可用。 |
| 🪶 | **极致轻量** | 单个约 35 KB 的文件：HTML + CSS + 原生 JavaScript，无构建步骤、无框架、无依赖。 |
...
```
> [!tip]
> 应用完全自包含——无 CDN、无外部请求，双击 `index.html` 即可离线使用。（`introduce/` 介绍页仍通过 CDN 加载 Tailwind/Lucide。）
...
* **编码方式**：标题与正文用分隔符拼接，先 UTF-8 编码再 base64（`TextEncoder` + `btoa`），并对外做 URL 编码（`encodeURIComponent`）——base64 中的 `+` / `/` 不再可能破坏链接。
* **快照生成**：即时生成一个极小的只读 HTML 页面，笔记内容以转义后的 JSON 注入（防止脚本注入），自带主题切换按钮和返回编辑器的链接。快照 100% 自包含（内联 CSS + 内联 SVG），断网也能打开。
* **长度保护**：离线快照超过约 180 万字符时自动禁用并说明原因，同时更短的在线链接依然可用；两者都超限时才弹出错误提示。

## 🛠️ 技术栈

* 原生 **HTML / CSS / JavaScript**（ES2015+），IIFE 作用域封装，不污染全局
* 手写 CSS + CSS 变量实现主题（无框架、无 CDN）
* 内联 Lucide 风格 SVG 图标（仅 5 个用到的，无外部图标库）
...
* 笔记过长时可能超出浏览器 URL 长度限制；离线快照过长时应用会自动回退到在线链接。
